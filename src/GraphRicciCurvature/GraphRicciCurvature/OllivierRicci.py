...
def _compute_ricci_curvature_single_edge(source, target):
    ...
    if _Gk.weight(source, target) < EPSILON:
        ...
        # logger.warning("Zero weight edge detected for edge (%s,%s), return Ricci Curvature as 0 instead." %(source, target))
...
def _compute_ricci_curvature_edges(G: nx.Graph, weight="weight", edge_list=[],
    ...
    #---------------task2------------
    # p1 = Pool(processes=_proc)
    # args_list2 = args[mid_num:len(args)]
    # result = p1.imap_unordered(_wrap_compute_single_edge, args_list2, chunksize=chunksize)
    # p1.close()
    # p1.join()
    # for rc in result:
        # for k in list(rc.keys()):
...
def _compute_ricci_flow(G: nx.Graph, weight="weight",
    ...
    surgery_func, do_surgery,input_name = surgery
    # input_name_index = input_name + "-1"
    ...
    for i in range(iterations+1):
        ...
        # name = "/mnt/f/test/code/curvature/GraphRicciCurvature/data/"+str(i)+input_name+".gml"
        input_name_index = input_name+str(i)
        ...
        if i != 0 and i % (do_surgery) == 0:
            ...
            name_surgery = str(int(i/do_surgery))
            print("==============name_surgery=============",name_surgery)
            # input_name_index = input_name + "-1"
            save_graph(G,input_name_index)
            # G = surgery_func(G, weight,fig_num = name_surgery,input_name=input_name)
            normalized_weight = float(G.number_of_edges())
            # k = str(int(i/do_surgery))
            # print("==============k=============",k)
            # G = surgery_func(G, weight,k)
            # normalized_weight = float(G.number_of_edges())
        ...
        # print (do_surgery,i % (do_surgery-1))
        # if i != 0 and i % (do_surgery) == 0:
            # k = str(int(i/do_surgery))
            # print("==============k=============",k)
            # G = surgery_func(G, weight,k)
            # normalized_weight = float(G.number_of_edges())
            # name = "club_graph_surgery_"+ str(i/(do_surgery-1)) +".gml"
            # temp_H = G.copy.deepcopy();
            # print(name)
            # nx.write_gml(G,name )
