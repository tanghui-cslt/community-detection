...
def _mod_compute_ricci_curvature_edge(G: nx.Graph, weight="weight", edge_list=[],
                                   alpha=0.5, method="OTD",
                                   base=math.e, exp_power=2)：
...
def _compute_ricci_flow(G: nx.Graph, weight="weight",
    ...
    for i in range(iterations+1):
        ...
        print("input_name :", input_name_index)
        ...
        if i != 0 and i % (do_surgery) == 0:
            ...
            G = surgery_func(G, weight,name_surgery)
            normalized_weight = float(G.number_of_edges())
