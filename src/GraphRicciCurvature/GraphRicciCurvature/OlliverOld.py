    ...
    def compute_ricci_curvature(self):
        ...
        for n in self.G.nodes():
            ...
            if self.G.degree(n) != 0:
                ...
                logger.debug("node %s, Ricci Curvature = %f" % (n, self.G.node[n]['ricciCurvature']))

    def compute_ricci_flow(self, iterations=100, step=1, delta=1e-4, reload=False,start_pos ='',surgery=(lambda G, *args, **kwargs: G, 100)):
        """
        ...
        save_graph(self.G,input_name_index)
        # surgery_func(self.G, self.weight,fig_num = input_name_index,input_name=input_name)
        ...
        for i in range(iterations+1):
            ...
            if i != 0 and i % do_surgery == 0:
                if reload == False:
                    ...
                    name_surgery = start_pos+"_"+str(int(i/do_surgery))+"_new_"
                
                
                self.G = surgery_func(self.G, self.weight,fig_num = name_surgery,input_name=input_name)
